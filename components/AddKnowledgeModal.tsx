...
interface AddKnowledgeModalProps {
  type: SourceType;
  onClose: () => void;
  onAdd: (data: { name: string; content?: { question: string; answer: string }}) => void;
}
...
const AddKnowledgeModal: React.FC<AddKnowledgeModalProps> = ({ type, onClose, onAdd }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'faq') {
        if (question && answer) {
            onAdd({ name: question, content: { question, answer }});
        }
    } else {
        if (name) {
            onAdd({ name });
  ...
  const renderFormFields = () => {
    switch (type) {
                ...
                <div className="space-y-1 text-center">
                    ...
                    <div className="flex text-sm text-gray-600 dark:text-gray-400">
                        <label htmlFor="file-upload" className="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none">
                            ...
                            <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={e => setName(e.target.files?.[0]?.name || '')} />
                        </label>
      ...
      case 'url':
        return (
          <div>
            <label htmlFor="url" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Website URL</label>
            <input
              type="url"
              id="url"
              value={name}
